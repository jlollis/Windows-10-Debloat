...
#
#WARNING: This script will neuter Windows 10.  It may cause irreversible changes, which you may not like.  It does exactly what it says, however I encourage you to research these changes beforehand. 
...
#
#PURPOSE: Eliminate all the garbage that comes with Windows. Change a vast amount of privacy settings in your favour. Remove built-in advertising. Disable most data collection. 
#         Shut Cortana down. Disable OneDrive. TAKE BACK WINDOWS.
#
#
#BEST RESULTS: Run this on a brand new machine from the first freshly created account, BEFORE you make any new users.  Any existing users other than the one you are using, will not have registry set for them.  
#              In that case, you can log into any existing users and run this using the -settingsonly switch.  BOOYA! How you like them apples?
#
#TIP: Run this in the audit stage of your custom image. Save adminsitration headaches messing with settings and apps. 
#
...
#-allapps       Includes store and calculator apps during app removal process.
#-leavetasks    Leaves scheduled tasks alone.
#-leaveservices Leaves services alone.
#-clearstart    Applies a clean start menu with only the File Explorer and Snipping Tool pinned (Have to have at least 1 app pinned to import a menu, I chose these two). Customize your pinned apps by editing the layout right in this script! (Find the ClearStartMenu function).
...
Function RemMostApps {
    If ($allusers) {  
        Write-Host "***Removing all apps and provisioned appx packages for this machine except Store, Photos, Sticky Notes, and Calculator...***"      
        ...
        Get-AppxPackage -AllUsers | where-object {$_.name -notlike "*Store*" -and $_.name -notlike "*Calculator*" -and $_.name -notlike "*Windows.Photos*" -and $_.name -notlike "*SoundRecorder*" -and $_.name -notlike "*MSPaint*" -and $_.name -notlike "*StickyNotes*"} | Remove-AppxPackage -erroraction silentlycontinue
        Get-AppxProvisionedPackage -online | where-object {$_.displayname -notlike "*Store*" -and $_.displayname -notlike "*Calculator*" -and $_.displayname -notlike "*Windows.Photos*" -and $_.displayname -notlike "*SoundRecorder*"  -and $_.displayname -notlike "*MSPaint*" -and $_.name -notlike "*StickyNotes*"} | Remove-AppxProvisionedPackage -online -erroraction silentlycontinue
       
}    Else {
        Write-Host "***Removing all apps for the current user, except Store, Photos and Calculator...***"                 
        Get-AppxPackage | where-object {$_.name -notlike "*Store*" -and $_.name -notlike "*Calculator*" -and $_.name -notlike "*Windows.Photos*" -and $_.name -notlike "*SoundRecorder*"} |  Remove-AppxPackage -erroraction silentlycontinue
        Get-AppxPackage | where-object {$_.name -notlike "*Store*" -and $_.name -notlike "*Calculator*" -and $_.name -notlike "*Windows.Photos*" -and $_.name -notlike "*SoundRecorder*"} |  Remove-AppxPackage -erroraction silentlycontinue
}        
}
...
Function RegSetUser {
    ...
    #Preinstalled apps, Minecraft Twitter etc all that crap - Enterprise only it seems
    ...
    #Filter web content through smartscreen
    ...
    #Don't ask for feedback, YOU DON'T WANNA HEAR IT TRUST ME
    ...
    #Stopping Cortana from remembering history"
...
Function RegSetMachine {
    ...
    #Don't search the web or dispaly web results in search			
